<template>
    <article class="flex flex-col w-full max-w-[676px] max-md:max-w-full">
      <header class="flex flex-col w-full text-gray-950 max-md:max-w-full">
        ...
        <p class="mt-4 text-xl tracking-tighter leading-5 max-md:max-w-full">{{ content }}</p>
      </header>
        ...
        <div class="flex overflow-hidden gap-px items-center self-stretch my-auto whitespace-nowrap rounded-[30px]">
          ...
          >
            <span class="flex gap-1 items-center self-stretch my-auto tracking-normal">
              <img :src="likeIcon" :alt="isLiked ? 'Unlike post' : 'Like post'" class="object-contain shrink-0 self-stretch my-auto aspect-[1.18] w-[13px]" />
              <span>Like</span>
            </span>
          ...
          >
            <span class="flex gap-1 items-center self-stretch my-auto tracking-normal">
              <img :src="dislikeIcon" :alt="isDisliked ? 'Remove dislike' : 'Dislike post'" class="object-contain shrink-0 self-stretch my-auto aspect-[1.18] w-[13px]" />
              <span>Trash</span>
            </span>
            <span :class="['self-stretch my-auto tracking-tighter text-right', !isDisliked && 'opacity-30']">
              {{ dislikes }}
            </span>
          </button>
        </div>
        <div class="flex gap-2 items-end self-stretch my-auto tracking-normal min-w-[240px]">
          <button @click="toggleComments" class="flex gap-1 items-center text-orange-500 w-[108px]">
          ...
          <div class="flex gap-1 items-start text-black whitespace-nowrap">
            ...
            <span class="self-stretch p-1 rounded bg-zinc-500 bg-opacity-10">american</span>
            <span class="self-stretch p-1 rounded bg-zinc-500 bg-opacity-10">crime</span>
          </div>
        </div>
      </footer>
    </article>
